    ...
    success: function(result) {
      ...
      if (dataTree == "Invalid URL") {
        ...
        for (var file in dataTree["files"]) {
          html +=
            '<div data-wms-url="' +
            dataTree["files"][file]["WMS"] +
            "" +
            '" data-subset-url="' +
            dataTree["files"][file]["NetcdfSubset"] +
            "" +
            '" data-opendap-url="' +
            dataTree["files"][file]["OPENDAP"] +
            "" +
            '" class="file" ' +
        ...
        for (var folder in dataTree["folders"]) {
          html +=
            '<div data-url="' +
            dataTree["folders"][folder] +
            '" class="folder" ' +
            'onclick="update_filepath.call(this)">' +
            '<p class="fas" style="display: inline-block">&#xf07b; ' +
            folder +
            "</p></div>"
        }
        $("#filetree-div").empty()
