    ...
    try {
        if ($("#new-credential-machine").val() === "" || $("#new-credential-username").val() === "" || $("#new-credential-password").val() === "") {
            ...
            if (resultMessage.errorMessage !== undefined) {
                ...
                ACTIVE_VARIABLES_PACKAGE.userAuthenticationCredentials[credentialId] = credentialArray;
        ...
        notifyOfDanger("An error occurred while adding the credential to the server.");
        console.error(error);
    }
};
...
removeCredentialFromServer = async function (credentialId) {
    try {
        const credentialArray = ACTIVE_VARIABLES_PACKAGE.userAuthenticationCredentials[credentialId];
